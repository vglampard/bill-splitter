...
import {
  getTotalPaid,
  tallyCheck,
  createBillingArrays, addPaymentRecipient
...
console.log("RECIPADDED:", addPaymentRecipient(billing) )
  ...
  function whoPaysWho(owersAndOwedRef, share, total) {
    ...
    let owersRes = owersAndOwedRef.owers.map((person) => ({
      ...person,
      pays: owersAndOwedRef.owed[0].name,
    }));
  ...
  function handleClick() {
    whoPaysWho(owersAndOwedRef, share, total);
          ...
          <div className="owedData">
            {owersAndOwedRef.owed.map((payer) => {
              return (
                <p>
                  {payer.name} is owed:{" "}
                  {Math.abs(calculateBalance(share, payer.amount))}
                </p>
              );
            })}
          </div>
